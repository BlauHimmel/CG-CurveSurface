...
public:
	...
	virtual void Paint(ArgParser* args)
	{
		...
		{	
			bezier_4_control_point(i, args->curve_tessellation);
		}	
	}
	...
	virtual void OutputBSpline(FILE *file)
	{
		...
		bspline.Inverse();
		Matrix transform = bezier * bspline;
		int num_splines = 0;
		Vec3f* splines[2500];
		for (int i = 0; i < vertices_num - 1; i += 3)
		{
	...
	{
		int line_each_di = ((float)vertices_num - 1.0f) / 3.0f;
...
private:
	void bezier_4_control_point(int start_index, int step)
	{
